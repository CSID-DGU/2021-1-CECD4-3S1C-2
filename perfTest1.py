...
totalPages = []
for x in range(len(texts_url)):
    url_ajax.append("https://apis.naver.com/commentBox/cbox/web_naver_list_jsonp.json?ticket=news&templateId=default_politics_m1&pool=cbox5&_callback=jQuery1707138182064460843_1523512042464&lang=ko&country=&objectId=news" +
                    oid[x] + "%2C" + aid[x] + "&categoryId=&pageSize=100&indexSize=10&groupId=&listType=OBJECT&pageType=more&page=1&refresh=false&sort=FAVORITE")
...
for x in range(len(texts_url)):
    resp = urlopen(
        Request(url_ajax[x], headers=headers[v])).read().decode('utf-8')
    news_json = getJson_NewsInfo(resp)
    totalPages = news_json['result']['pageModel']['totalPages']
    comps_url = []
...
# for x in range(len(texts_url)):
#     comps_url = []
#     for y in range(1, totalPages[x]+1):
#         comps_url.append("https://apis.naver.com/commentBox/cbox/web_naver_list_jsonp.json?ticket=news&templateId=default_politics_m1&pool=cbox5&_callback=jQuery1707138182064460843_1523512042464&lang=ko&country=&objectId=news" +
#                          oid[x] + "%2C" + aid[x] + "&categoryId=&pageSize=100&indexSize=10&groupId=&listType=OBJECT&pageType=more&page=" + str(y) + "&refresh=false&sort=FAVORITE")
#     com_url.append(comps_url)
# print(com_url[0][1])
